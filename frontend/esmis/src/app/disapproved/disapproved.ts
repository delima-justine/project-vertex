...
export class Disapproved implements OnInit {
  ...
  loadDisapprovedRequests(page: number = 1) {
    this.isLoading.set(true);
    this.supplyService.listSupplyRequests('disapproved', undefined, page).subscribe({
      next: (response) => {
      ...
      error: (err) => {
        console.error('Error fetching disapproved requests', err);
        this.isLoading.set(false);
      }
    });
  }
  ...
  changePage(page: number) {
    if (page >= 1 && page <= this.lastPage()) {
      this.loadDisapprovedRequests(page);
    }
  }
